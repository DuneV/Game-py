...
class SL():
    ...
    def pru(self):
        pygame.draw.circle(self.screen, GREEN, (125, 375), self.radius)

    def collision(self):
        flag = False
        
        for i in range(len(self.objectsx)):
            if self.x + 25 == int(self.objectsx[i]) and self.y + 25 == int(self.objectsy[i]):
                flag = True
            else:
                flag = False
